...
router.post('/cadastro', (req, res) => {
  ...
  connection.query('INSERT INTO cadastro (nome, email, cpf,  telefone, senha) VALUES (?, ?, ?, ?, ?)', 
    [nome, email, cpf,  telefone, senha], (err, result) => {
    if (err) {
...
router.put('/cadastro/:id', (req, res) => {
  const { id } = req.params;
  const { nome, email, cpf,  telefone, senha } = req.body;
  connection.query('UPDATE cadastro SET nome = ?, email = ?, cpf = ?,  telefone = ?, senha = ? WHERE id = ?', 
    [nome, email, cpf, telefone, senha, id], (err, result) => {
    if (err) {
